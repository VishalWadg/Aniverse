...
public interface PostRepository extends JpaRepository<Post, UUID> {
       ...
       @Query(value = "SELECT p FROM Post p JOIN FETCH p.author WHERE p.isDeleted = false", countQuery = "SELECT count(p) FROM Post p WHERE p.isDeleted = false")
       Page<Post> findActiveWithAuthor(Pageable pageable);

       @Query(value = "SELECT p FROM Post p JOIN FETCH p.author WHERE p.author.username = :username AND p.isDeleted = false", countQuery = "SELECT count(p) FROM Post p WHERE p.author.username = :username AND p.isDeleted = false")
       ...
       @Query(value = "SELECT p FROM Post p JOIN FETCH p.author WHERE p.isDeleted = true", countQuery = "SELECT count(p) FROM Post p WHERE p.isDeleted = true")
       ...
       @Query("SELECT p FROM Post p WHERE p.isDeleted = true AND p.deletedAt < :cutoff")
List<Post> findExpiredPostsForPurge(@Param("cutoff") LocalDateTime cutoff);
       ...
       @Query("SELECT p.content FROM Post p WHERE p.isDeleted = true AND p.deletedAt < :cutoff AND p.content IS NOT NULL")
       List<String> findExpiredPostContents(@Param("cutoff") LocalDateTime cutoff);

       @Query(value = "SELECT p FROM Post p JOIN FETCH p.author WHERE " +
                     "p.isDeleted = false AND " +
                     "(LOWER(p.title) LIKE LOWER(CONCAT('%', :query, '%')) OR " +
                     "LOWER(p.content) LIKE LOWER(CONCAT('%', :query, '%')))", countQuery = "SELECT count(p) FROM Post p WHERE "
                                   +
                                   "p.isDeleted = false AND " +
                                   "(LOWER(p.title) LIKE LOWER(CONCAT('%', :query, '%')) OR " +
       ...
       List<String> findAllPostContents();
