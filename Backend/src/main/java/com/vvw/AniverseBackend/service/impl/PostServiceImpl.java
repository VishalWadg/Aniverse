...
public class PostServiceImpl implements PostService {
    ...
    private Post findActivePostOrThrow(UUID id) {
        return postRepository.findActiveByIdWithAuthor(id)
    ...
    // PUT Request (All fields are required) mostly not needed as we use PATCH
    // instead
    @Override
    @Transactional
    ...
    // PATCH Request all fields are not required(not notnull) and only fields with
    // notnull value are updated others are kept as is
    // this can be done two ways using 1) Map<String, Object> and 2) using DTO with
    // null values allowed fields 2nd approach is preferred
    @Override
    @Transactional
    ...
    public Boolean isUserTheAuthor(String username, UUID post_id) {
        return postRepository.findAuthorUsernameByPostId(post_id).map(authorUsername -> authorUsername.equals(username))
                .orElse(false);
    }
    ...
    public void purgeDeletedPost(UUID id) {
        ...
        // Extract public IDs BEFORE deletion
        Set<String> publicIds = PostImageExtractor.extractPublicIds(post.getContent());

        // Atomic DB Hard Delete
        int count = postRepository.hardDeleteById(id);
        if (count != 1) {
            // If count == 0, the post was either restored concurrently or deleted by
            // another admin
            throw new InvalidOperationException("Failed to permanently delete post with id " + id);
        }

        // Best-effort Cloudinary image cleanup ONLY if DB deletion succeeded (count ==
        // 1)
        publicIds.forEach(publicId -> {
            try {
                cloudinaryService.deleteFile(publicId);
            } catch (Exception e) {
                log.warn("Failed to delete Cloudinary asset {} for purged post {}. Scheduled job will self-heal.",
                        publicId, id, e);
            }
        });
    ...
    public int purgeExpiredDeletedPosts() {
        ...
        List<Post> expiredPosts = postRepository.findExpiredPostsForPurge(cutoff);
        ...
        for (Post post : expiredPosts) {
            int count = postRepository.hardDeleteById(post.getId());
            if (count == 1) {
                totalPurged++;
                Set<String> publicIds = PostImageExtractor.extractPublicIds(post.getContent());
                publicIds.forEach(publicId -> {
                    try {
                        ...
                        log.warn(
                                "Failed to delete Cloudinary asset {} for purged post {}. Scheduled job will self-heal.",
                                publicId, post.getId(), e);
                    }
                });
            } else {
                log.warn("Post {} was restored concurrently during purge; skipped image cleanup.", post.getId());
            }
        }

        log.info("Expired post purge completed. Purged {} posts from DB.", totalPurged);
        return totalPurged;
    }
